...
export const HotelsList = () => {
  ...
  const [hotel, setHotel]=useState<IHotel>({id:"",
      title:"",
      description: "",
    })
...
const handleHotel = (data:IHotel) => {
  ...
  useEffect(() => {
    async function fetchData() {
        await dispatch(GetDataFromApiThunk(GetAllHotels({offset: 0, limit: ROWS_PER_PAGE })));
      setLoaded(true);
    }
    fetchData();
  }, []);
                  ...
                  <table className={classes["table-wrap"]}>
                    ...
                    <tbody>
                      {data.map((item:IHotel, index: number) => (
                        <tr key={item.id}>
                          <td >{index + 1}</td>
                          <td className={classes["table-cell"]}>{item.title}</td>
                         <td className={classes["table-cell"]}>{item.description}</td>
                        <button className={classes["trash-button"]} onClick={()=>{
                          handleHotel(item)}}> 
                        <i>{editIcon}</i>
                        </button>
                        </tr>
