...
export const InputField = (props) => {
  const { className, type, name, value, placeholder, onChange, isError } =
    props;
  const iconAlert = (
    ...
    <><div className={classes["messageBlock"]} >
      <div
        className={`${className} ${classes["formField"]} ${
          isError ? classes["errorField"] : classes["checkedField"]
        }`}
      >
        <input
          type={type}
          ...
          onChange={onChange}
        />
        <i className={classes["icon"]}>{isError ? iconAlert : iconPass}</i>
      </div>
